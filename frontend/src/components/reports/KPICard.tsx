...
export const KPICard: React.FC<KPICardProps> = ({
  ...
  return (
    <div
      className={`rounded-xl p-6 bg-[#1F2937] ${
        variant === 'totalizador' ? 'border-l-[3px] border-l-[#7C3AED]' : ''
      }`}
    >
      {/* Título */}
      <h3 className="text-base font-medium text-gray-400 mb-3">{title}</h3>
      ...
      {changeIndicator && (
        <div className="flex items-center gap-1.5 mt-3">
          ...
          <span className="text-sm text-gray-400">{changeIndicator.label}</span>
        </div>
      )}
        ...
        <div className="mt-4 space-y-2">
          {subLabels.map((sub, index) => (
            <p key={index} className="text-sm text-gray-400">
              {sub.label}: <span className="text-white font-medium">{sub.value}</span>
            </p>
          ))}
        </div>
      )}
